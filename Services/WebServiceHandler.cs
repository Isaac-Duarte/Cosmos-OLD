...
namespace Cosmos.Services
{
	...
	public class WebServiceHandler {
		...
		public async Task<XDocument> CallAsync(XDocument xml, string method = "POST", string functionName = null) {
			return
				await CallAsync(
					xml.ToString(),
					method,
		...
		/// <param name="functionName">Name of function. (only for logging purposes)</param>
		/// <returns>Response XML, if any.</returns>
		public async Task<XDocument> CallAsync(string xml, string method = "POST", string functionName = null) {
			...
			request.Headers["SOAPAction"] = functionName;
			...
			try {
				...
				resXml = await XDocument.LoadAsync(response.GetResponseStream(), LoadOptions.None, new System.Threading.CancellationToken());
			}
